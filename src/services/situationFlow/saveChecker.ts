...
export const saveChecker = (nodes: Node[], formStoreRef: {title: string, description: string} | null, setIsSaveDisabled: React.Dispatch<React.SetStateAction<IsSaveDisabled>>) => {
	...
	const hasTitleAndDescription = formStoreRef?.title && formStoreRef?.description;
	const hasMinimumQuestionAndAnswers = questionNodes.length >= 1 && answerNodes.length >= 2;
