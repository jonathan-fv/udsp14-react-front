...
export const normalizeFlowData = ({ flowData, formStoreRef }: NormalizeFlowDataProps) => {
	...
	const filteredArray = updatedNodes.map((obj: { data?: any; edges?: any; id?: any; type?: any; }) => {
		const { id, type } = obj;
		const { label } = obj.data;
		const targets = obj.edges
			.map((edge: { target: any; }) => edge.target)
			.filter((target: any) => {
				const targetObj = updatedNodes.find((node: { id: any; }) => node.id === target);
				return targetObj?.type !== 'image' && targetObj?.type !== 'sound';
			});

		const media = obj.edges
			.filter((edge: { target: any; }) => {
				const targetObj = updatedNodes.find((node: { id: any; }) => node.id === edge.target);
				return targetObj?.type === 'image' || targetObj?.type === 'sound';
			})
			.map((edge: { target: any; }) => {
				const targetObj = updatedNodes.find((node: { id: any; }) => node.id === edge.target);
