...
const CreateSituationFlow = ({ situationId }: SituationFlowProps) => {
	...
	const formStoreRef = useRef<{ title: string; description: string } | null>(
		null
	);
	...
	const handleFormUpdate = useCallback(
		(formStore: { title: string; description: string }) => {
			...
			if (['image', 'sound'].includes(type as string)) {
				if (selectedNode.data.url) {
					...
					try {
						...
						console.log(`http://localhost:8000/upload/${path}/?name=${fileName}`)
					} catch (e) {
						console.log(e)
					}
				}
	...
	return (
		<div className="dndflow">
			<ReactFlowProvider>
				...
				<Sidebar
					{...{ onSave, onRestore, onDelete, selectedNode, isSaveDisabled }}
				/>
			</ReactFlowProvider>
		</div>
	);
